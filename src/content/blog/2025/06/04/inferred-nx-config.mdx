...
The exact code to make this transformation isn't particularly interesting. You can borrow my implementation: [getInferredBuildOptions.ts](https://github.com/bgschiller/inferred-nx-tasks/blob/main/packages/shared-nx-plugins/src/shared/getInferredBuildOptions.ts)
...
Nah, just kidding. Take a look at [config.ts](https://github.com/bgschiller/inferred-nx-tasks/blob/main/packages/shared-nx-plugins/src/executors/rollup/config.ts) for a complete implementation of `createRollupOptions`, including definitions for `createExternalOption` and `createOutputOptions`.
